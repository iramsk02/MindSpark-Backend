...
const clients = new Map<CustomWebSocket, { courseId: string, username: string }>();

wss.on("connection", (ws: CustomWebSocket, req: IncomingMessage) => {
  console.log("New client connected");
  try {
    const user = verifySocketToken(req); // you may need to adjust this
    ws.user = user;
    clients.set(ws, { courseId: "", username: "" }); // Empty initially
  } catch (err) {
    console.error("WebSocket auth failed:", err);
    ws.close(4001, "Unauthorized");
    return;
  ...
  ws.on("message", async (data) => {
    try {
      ...
      const { type, courseId, message, user } = parsed;
      const userr = await userModel.findById({ _id: user });
      const username = userr?.name || "Anonymous";
      ...
      if (type === "message") {
        ...
        wss.clients.forEach((client) => {
          ...
          if (client.readyState === WebSocket.OPEN && clients.get(c)?.courseId === courseId) {
            client.send(JSON.stringify({ type: "message", username, message }));
          }
        });
      }
